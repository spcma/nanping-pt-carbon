...
type MethodologyHandler struct {
	appService *MethodologyAppService
}

// NewMethodologyHandler creates methodology handler
func NewMethodologyHandler(appService *MethodologyAppService) *MethodologyHandler {
...
func (h *MethodologyHandler) Create(c *gin.Context) {
	var cmd CreateMethodologyCommand
...
func (h *MethodologyHandler) GetPage(c *gin.Context) {
	var query MethodologyPageQuery
	if err := c.ShouldBindQuery(&query); err != nil {
		response.Error(c, http.StatusBadRequest, err.Error())
		return
	}
...
func (h *MethodologyHandler) ChangeStatus(c *gin.Context) {
	...
	user := platform_http.GetCurrentUser(c)
	changeCmd := ChangeMethodologyStatusCommand{
		ID:     id,
		Status: MethodologyStatus(cmd.Status),
		UserID: user.ID,
	}
