...
func (s *ProjectMembersService) CreateProjectMember(ctx context.Context, param *CreateProjectMemberParam) (int64, error) {
	...
	for _, member := range projectMembers {
		if member.ProjectId == param.ProjectId {
			return member.Id, ErrProjectMemberAlreadyExists
		}
	}

	member, err := NewProjectMembers(param.ProjectId, param.UserId, param.CreateBy, param.Role)
	if err != nil {
		return 0, err
	}
...
func (s *ProjectMembersService) GetProjectMemberByID(ctx context.Context, id int64) (*ProjectMembers, error) {
	return s.repo.FindByID(ctx, id)
}

// GetProjectMembersByProjectID 根据项目 ID 获取成员列表
func (s *ProjectMembersService) GetProjectMembersByProjectID(ctx context.Context, projectID int64) ([]*ProjectMembers, error) {
	return s.repo.FindByProjectID(ctx, projectID)
}

// GetProjectMembersByUserID 根据用户 ID 获取参与的项目
func (s *ProjectMembersService) GetProjectMembersByUserID(ctx context.Context, userID int64) ([]*ProjectMembers, error) {
	return s.repo.FindByUserID(ctx, userID)
}

// GetProjectMemberByProjectAndUser 根据项目和用户获取成员信息
func (s *ProjectMembersService) GetProjectMemberByProjectAndUser(ctx context.Context, projectID, userID int64) (*ProjectMembers, error) {
