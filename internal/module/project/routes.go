...
func NewProjectRoutes() shared_http.RouteRegistry {
	...
	//	初始化 project 模块
	projectRepo := NewProjectRepository(dbInst)
	appService := NewProjectService(projectRepo)
	projectHandler := NewProjectHandler(appService)

	//	初始化 projectMembers 模块
	projectMembersRepo := NewProjectMembersRepository(dbInst)
	projectMembersAppService := NewProjectMembersService(projectMembersRepo)
