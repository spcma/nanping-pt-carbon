...
func (s *ProjectAppService) Create(ctx context.Context, cmd CreateProjectCommand) (int64, error) {
	...
	project, err := NewProject(cmd.Name, cmd.Code, "", cmd.Description, cmd.UserID, timeutil.Now(), timeutil.Now())
	if err != nil {
		return 0, err
	}
...
func (s *ProjectAppService) GetByQuery(ctx context.Context, query *ProjectQuery) (*Project, error) {
	if query == nil {
...
func (s *ProjectAppService) GetList(ctx context.Context) ([]*Project, error) {
...
// GetPage 分页查询项目
func (s *ProjectAppService) GetPage(ctx context.Context, query *ProjectPageQuery) (*entity.PaginationResult[Project], error) {
